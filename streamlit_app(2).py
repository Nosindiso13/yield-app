...
import sqlite3 # New: For SQLite database interaction
...
DATABASE_PATH = 'users.db' # New: Path for SQLite database
...
def init_db():
    with sqlite3.connect(DATABASE_PATH) as conn:
        ...
        ''')
        # Add a default admin user if the database is empty (for first run convenience)
        # In a real app, you'd have a better way to provision initial users.
        ...
        if cursor.fetchone() is None:
            ...
            conn.commit()
            st.info("Default admin user created: username='admin', password='adminpass'")
        conn.commit()
